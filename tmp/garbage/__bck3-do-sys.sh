...
readonly _src_dir="/"
readonly _dst_mnt_name="storage-v";
readonly _dst_mnt_path="/mnt/${_dst_mnt_name}";

if [ ! $_dst_mnt_path == $(lsblk -l -o MOUNTPOINT | awk '/^\/mnt\/storage-v$/ {print $1}') ];
then
...
readonly _backup_dir="${_dst_mnt_path}/backup-sys/$(hostname)";
readonly _backup_path="${_backup_dir}/${_datetime}";
readonly _latest_link="${_backup_dir}/latest";
readonly _log_file="${HOME}/__backup/logs/backup-sys.${_datetime}.txt";
readonly _exclusion_file="${HOME}/__backup/__backup-sys-exclusions.txt";
_prev_backup_path="";
