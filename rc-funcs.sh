...
function g_tag() {
    _info "list tag matching grep pattern '$1'..."

    git tag -l | grep -E "$1" | sort

    _info "all done"
}

function g_del_tag() {
    _info "delete tag matching grep pattern '$1'..."
    ...
    for t in $(echo $tags); do
        git tag -d $t
        if [ $? -ne 0 ]; then
            _warn "error while deleting tag '$t'"
        fi
    done
...
function g_msg() {
    local msg=$(git log -n 1 --format=%B)
    if [ $? -ne 0 ]; then
        return 1
    fi
    echo $msg
