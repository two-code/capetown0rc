...
readonly _ikey="${__secrets}/bck7-ikey.gpg"
readonly _ikey_raw="${_ramfs_mnt_path}/ikey"
readonly _key="${__secrets}/bck7-key.gpg"
readonly _header="${__secrets}/bck7-header.gpg"
...
readonly _back_dev="/dev/$(basename $(readlink /dev/disk/by-partlabel/bck7-vault))"
...
echo -e "${__color_yellow}open integrity-device...${__color_none}" &&
    ...
    gpg2 --verbose --output "${_ikey_raw}" --decrypt "${_ikey}" &&
    ...
    gpg2 --verbose --output "${_header_raw}" --decrypt "${_header}" &&
    (gpg2 --verbose --decrypt "${_key}" | sudo cryptsetup --key-file=- --key-slot=4 --header="${_header_raw}" --perf-no_read_workqueue --perf-no_write_workqueue --persistent luksOpen "${_imap_full}" "${_emap}") &&
